...
class mod_scorm_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        $decodeEmial = urldecode($userEmail);
        setrawcookie("email", urldecode($userEmail), 0, "/", ".isasync.pl");

        // Summary.
        $this->standard_intro_elements();
        echo("<script src='https://cdnjs.cloudflare.com/ajax/libs/axios/1.0.0-alpha.1/axios.min.js'></script>");
        echo("<script>

        gen = false;
        function showCreator(){
            const div = document.getElementById('iframeDiv')

            const btn = document.getElementById('btnShowCreator')
            ...
            if(!gen){
                ...
                //iframe.src = 'https://adapt-tool.wspia.eu/#user/login'
                iframe.src = 'https://adapt-tool.wspia.eu'
                div.append(iframe)
            ...
            if(div.style.display === 'none'){
                document.getElementById('iframe').src = 'https://adapt-tool.wspia.eu'
                div.style.display = 'block'

                btn.className = 'btn-danger btn btn-block w-25 mb-4'

                btn.innerHTML= 'Wyłącz kreator'

            }else{

                div.style.display = 'none'

                btn.className = 'btn-primary btn btn-block w-25 mb-4'

                btn.innerHTML= 'Otwórz kreator'
        ...
        <button type="button" id="btnShowCreator" class="btn-primary btn btn-block w-25 mb-4" onclick="showCreator()">Otwórz kreator</button>
        </div>
        <div id="iframeDiv" class="mb-4" style="height: 800px; display:none"></div>'
    );
