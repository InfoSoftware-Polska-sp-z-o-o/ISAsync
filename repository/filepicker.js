       ...
       classnamecallback : function(node) {
        ...
        create_upload_form: function(data) {
            ...
            content.one('.fp-upload-btn').on('click', function(e) {
				...
				if(plCharChecker(content.one('.fp-saveas input').get('value'))){
					scope.print_msg('Nazwa nie może zawierać polskich znaków', 'error');
                    return false;
				}
                this.hide_header();
                scope.request({
...
function plCharChecker(str){
	const plArray = ['ą', 'ć', 'ę', 'ł', 'ń', 'ó', 'ś', 'ź', 'ż'];
	const contains = plArray.some(element => {  
