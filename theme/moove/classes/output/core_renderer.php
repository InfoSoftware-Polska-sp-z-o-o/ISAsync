...
class core_renderer extends \theme_boost\output\core_renderer {
    ...
    public function user_menu($user = null, $withlinks = null) {
        ...
        if ($withlinks) {
			...
			if(!empty($idnumber)) {
				$subtitle .= '<div style="display: flex;align-items: center;justify-content: space-around;"><small style="">Nr Indeksu:  ' . $idnumber . '</small></div>';
			};
			
			foreach ($opts->navitems as $value) {
                $usertitle = $value -> title;
                $idtitle = '<div style="display: flex;align-items: center;justify-content: center;">' . $usertitle . '</div>' . $subtitle;
